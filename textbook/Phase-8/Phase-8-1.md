...
>
> **[Phase 15-3 で確定 ── 実際に置換した]** Phase 15 の大規模入力テストで、この DFS版が
> 線形依存チェーン n≈999 から実際に `RecursionError` でクラッシュすることを実測で確認し、
> 上記の「置換時の影響は閉じる」という見立てのとおり Kahn 法へ置換した(60テストが1件の
> 出力順アサーション更新のみで無回帰)。詳細 `Phase-15-3.md`、`Phase-8-introduction.md` §12。
