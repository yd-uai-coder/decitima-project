...
| A8 | `registry.py` が後の章で作る `dijkstra.py` を import し、章を写経した時点でテストが赤 | 1 | 前方参照は import ごとコメントアウト + マーカー、参照先を作る章がコメント解除 + 配線テスト。機構テストはフェイクで(Q9) | ルール化(#15) |
...
## 3. 次の判断ポイント(Phase 7 着手前)

- Phase 7 以降を「学習モード」で続けるか、期間目安を切って「納期モード」を試すか(§2.1)。
- 提案 A(目安工数)/ C(事前質問を設計判断に限定)/ D(写経レベル)を Phase 7 教材から試験導入するか。
- CLAUDE.md スリム化(Step 1・2)は完了(§2.2 / Q39)。Phase 7 の教材・サンプルはこの前提で生成する
  ── 共有 `textbook/samples/` に `#(Phase 7-M)` タグ + 旧コードのコメントアウトで追記、章冒頭に「作成 / 更新するファイル」。
- `textbook/appendix/library-fork-impact.md` §5 のフックとの突き合わせ ── Phase 7 の
  Floyd-Warshall は「距離行列を返すプリミティブ」で、そこは NumPy 配列がむしろ自然
  (§2「置き換え表面」の Phase 7 行)。§8 の前提(コア層に NumPy を入れない)と整合するか確認。
...
| 現行の教材・コードに手を入れる必要は | 無い。本書は棚卸しであって改訂ではない(#12 の対象外) | 冒頭 blockquote |
