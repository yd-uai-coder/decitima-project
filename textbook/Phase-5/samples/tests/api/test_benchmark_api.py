...
async def test_benchmark_network_design(api) -> None:
    client, _user = api
    resp = await client.post("/api/v1/benchmark", json=_payload(build_network_problem(), runs=1))
    assert resp.status_code == 200
    body = resp.json()
    assert {e["algorithm"]["name"] for e in body["entries"]} == {"kruskal", "prim"}
    assert all(e["metrics"]["total_weight"] == 10.0 for e in body["entries"])
