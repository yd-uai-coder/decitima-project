...
> **[Phase 6 で確定 ── 実装済み]** 当初この置き場のロジックは「Phase 1〜2」としていた。
> 実際は `constraints/` のチェッカーは **Phase 2-3 で実装済み**、`objectives/` の
> `weighted_sum`(重み付き和の評価器)は **Phase 6-1 で実装済み**(初の多目的ストラテジー =
> Shift Scheduler の実装時)。Phase 1 の Validation / Verification は route 限定の最小実装のみ。
> 詳細は `Phase-6-1.md` / `Phase-1-introduction.md` §7 / `Phase-1-7.md` §5 / `Phase-2-3.md`、
> `Phase-0-2.md` §2.5 の同マーカー。
