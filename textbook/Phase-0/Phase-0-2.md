...
| Project Manager(Phase 8) | `ProjectData` / `ProjectSolution` を定義しユニオンに追加(Phase 8-3 で実装 ✅ ── §8.1 と同型。閉路検出は `validation.py`) | なし                 |
| Logistics Optimizer(Phase 9) | `LogisticsData` / `LogisticsSolution` を定義しユニオンに追加(Phase 9-1 で実装 ✅ ── §8.1 と同型。到達可能性検出は `validation.py`。CVRP + PuLP MILP + ジョブキュー) | なし                 |
