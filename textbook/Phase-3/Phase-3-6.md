# Phase 3-6: 比較テーブル + 入力サイズ曲線チャート + ページ(作業単位 3-6)
...
export default function BenchmarkPage() { return <BenchmarkPanel />; }
```
...
これで Phase 3 は完了。`Phase-3-introduction.md` の「次のフェーズ」を確認し、
「Phase 4 を開始する」で Route Planner / Network Designer(Bellman-Ford / A* / MST /
`networkx` 導入 / 経路可視化 ── ここで本格的な図ライブラリを選ぶ)の教材を生成する。
