# Phase 3-7: 分析トラック `analysis/`(pandas 導入)(作業単位 3-7)

## この章のゴール

`benchmark_runs` に貯まった実測を **pandas で集計・可視化する土台**を作る。
これは Phase 3 限定でなく、Phase 4/5/9/11/13/14 が同じ `analysis/` に足していく**多 Phase の
背骨**(ロードマップは `analysis/README.md`)。3-7 はその起点。
...
def load_solutions(path) -> pd.DataFrame:
    """solutions の JSONL を DataFrame に(Phase 13 の布石。metrics を metric_<key> 列に展開)。"""
```

- `load_solutions` は Phase 3 では使わないが、Phase 13(LLM vs Algorithm)が `solutions` を
...
def regression(baseline, current, *, metric="elapsed_ms_median") -> pd.DataFrame:
    """同一アルゴリズムの baseline → current の変化率(ratio / pct_change)。Phase 14 の CI 回帰検知用。"""
```

- **benchmark 専用モジュール**。後続 Phase は横に `analysis/shift_analysis.py`(Phase 5)、
  `analysis/experiment.py`(Phase 13)を並べる。
...
- `analysis/` は Phase 4/5/9/11/13/14 が育てる器。3-7 はその土台と benchmark 分析を置く。
...
これで Phase 3 は完了(6 章 + 3-7)。`Phase-3-introduction.md` の「次のフェーズ」を確認し、
