...
- ui: `src/lib/api/types.ts` / `src/lib/menu-tree.ts` / `src/components/auth/auth-store.ts` /
  `src/app/(pages)/optimization/benchmark/page.tsx` は **現行版を samples に同梱**
  (追記・改訂だが全体を写す方が安全 ── Phase 1 の教訓)
...
| `scripts/**`            | `decitima-api/backend/scripts/**`(3-5 ── dev シード)  |
...
| 3-3 | `app/services/benchmark.py`、`app/api/routes/benchmark.py`、`alembic/versions/d4f1a9c2b8e7_add_benchmark_runs_table.py`、`tests/unit/test_benchmark_service.py`、`tests/unit/test_benchmark_repository.py`、`tests/api/test_benchmark_api.py`、`tests/integration/test_benchmark_persistence.py` | `app/models/optimization.py`(`BenchmarkRun`・現行版同梱)、`app/repositories/optimization.py`(`BenchmarkRunRepository` 同居・現行版同梱)、`app/services/optimization_read.py`(`get_benchmark_run`・現行版同梱)、`app/core/config.py`、`app/api/routes/__init__.py`、`app/models/__init__.py`、`alembic/env.py` | Phase-3-3 |
...
| 3-5 | `ui/src/components/auth/{auth-api.ts,LoginForm.tsx}`、`ui/src/app/(pages)/login/page.tsx`、`ui/src/components/auth/{auth-api.test.ts,LoginForm.test.tsx}`、`scripts/{__init__.py,seed.py}` | `ui/src/components/auth/auth-store.ts`(`refreshTokens()` 修正・現行版同梱)、`ui/src/lib/api/types.ts`(`TokenPair`/`AccessToken` 追加・現行版同梱) | Phase-3-5 |
| 3-6 | `ui/src/features/optimization/{api/benchmark.ts,stores/benchmark-store.ts,hooks/useBenchmark.ts,sample-problems.ts}`、`ui/src/features/optimization/stores/benchmark-store.test.ts`、`ui/src/features/optimization/api/benchmark.test.ts`                                                                                    | `ui/src/lib/api/types.ts`(benchmark DTO 追加・現行版同梱)                                                                                                                                                            | Phase-3-6 |
| 3-7 | `ui/src/components/ui/charts/{GroupedBarChart,MultiLineChart}.tsx`、`ui/src/features/optimization/components/*.tsx`、`ui/src/app/(pages)/optimization/benchmark/page.tsx`(`RequireAuth` で包む)、`ui/src/components/ui/charts/GroupedBarChart.test.tsx`                                                                                             | `ui/src/lib/menu-tree.ts`(現行版同梱)                                                                                                                                                                              | Phase-3-7 |
| 3-8 | `analysis/{__init__,db,export,loaders,benchmark_report,plots}.py`、`analysis/{README.md,data/.gitignore,data/sample_benchmark_runs.jsonl,notebooks/benchmark_explore.ipynb}`、`tests/analysis/{test_loaders,test_benchmark_report,test_plots,test_export}.py`                                                                | `pyproject.toml`(`[dependency-groups].analysis` + ruff `src`/`known-first-party` に `analysis`)、`.gitignore`                                                                                                   | Phase-3-8 |
...
| `pyproject.toml`             | `[dependency-groups]` に `analysis = ["pandas>=2.2", "matplotlib>=3.9"]`。`[tool.ruff].src` と `[tool.ruff.lint.isort].known-first-party` に `analysis` を追加 | Phase-3-8 §7 |
| `.gitignore`                 | `analysis/data/*` を無視し `.gitignore` と `sample_benchmark_runs.jsonl` だけ許可                                                                                | Phase-3-8 §7 |
...
- `textbook/Phase-1/samples/app/repositories/optimization.py` ──
  `# [以降 Phase で修正予定 ── Phase 3-3]`(`BenchmarkRunRepository` 同居。現行版
  `textbook/Phase-3/samples/app/repositories/optimization.py`)
...
rsync -a textbook/Phase-3/samples/analysis/ <work>/analysis/   # 3-8
rsync -a textbook/Phase-3/samples/scripts/  <work>/scripts/    # 3-5(dev シード)
...
uv run ruff check app tests analysis scripts   # All checks passed
uv run ruff format --check app tests analysis scripts
uvx pyright app tests scripts              # 0 errors(analysis/ は ruff のみ ── pyright include 外)
DATABASE_URL="sqlite+aiosqlite:///./x.db" REDIS_URL=... JWT_SECRET_KEY=... \
  uv run alembic upgrade head              # benchmark_runs が生える
DATABASE_URL="sqlite+aiosqlite:///./x.db" REDIS_URL=... JWT_SECRET_KEY=... \
  uv run python -m scripts.seed            # テストユーザー作成 → 2 回目は "already exists (skip)"
...
npx vitest run src/components/auth src/features/optimization \
  src/components/ui/charts/GroupedBarChart.test.tsx
                                           # 11 passed(3-5 の auth 3 件 + 3-6 の 4 件 + 3-7 の 2 件 ほか)
npx eslint src/components/auth src/features/optimization src/components/ui/charts \
  src/lib/menu-tree.ts src/lib/api/types.ts \
  'src/app/(pages)/login/page.tsx' 'src/app/(pages)/optimization/benchmark/page.tsx'   # clean
```
...
3-8 注意: `analysis/` は **ruff のみ**(pyright `include` は `["app", "tests"]` のまま ──
