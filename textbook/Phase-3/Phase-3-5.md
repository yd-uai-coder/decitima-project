# Phase 3-5: 可視化基盤の選定 + src/features/optimization/ 骨格 + api 層の型付け(作業単位 3-5)
...
```typescript
// src/lib/api/types.ts(現行版。既存の AsyncStatus は残す)
...
- `OptimizationProblem` は**ベンチ要求に必要な最小形**だけ(route_planning のみ。shift は Phase 5)。
...
次章([Phase-3-6](./Phase-3-6.md))では、作業単位 3-6 ── 汎用の軸付きチャート 2 つと、
