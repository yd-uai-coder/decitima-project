# Phase 1-1: 概観 ── 決定論的な計算基盤と solve ライフサイクル
...
本 Phase の各章はこの単位に対応する。

| 単位 | 内容 | 章 | 依存 |
| --- | --- | --- | --- |
| 1-1 | 共通スキーマの実装(`app/domain/`) | [Phase-1-2](./Phase-1-2.md) | ― |
| 1-2 | `AlgorithmStrategy` プロトコル + `registry` + `select_strategy` | [Phase-1-3](./Phase-1-3.md) | 1-1 |
| 1-3 | 探索プリミティブ(Linear / Binary Search・BFS・DFS) | [Phase-1-4](./Phase-1-4.md) | ― |
| 1-4 | `DijkstraStrategy`(禁止エッジ・必須経由対応) | [Phase-1-5](./Phase-1-5.md) | 1-1, 1-2, 1-3 |
| 1-5 | `Problem` / `Solution` の ORM とリポジトリ + マイグレーション | [Phase-1-6](./Phase-1-6.md) | ― |
| 1-6 | `SolveService` + `POST /api/v1/solve` + 最小 Validation/Verification | [Phase-1-7](./Phase-1-7.md) | 1-1〜1-5 |
...
├── Phase-1-1.md 〜 1-7.md    各作業単位の解説(この一式)
└── samples/
...
次章([Phase-1-2](./Phase-1-2.md))では、作業単位 1-1 ── 共通スキーマを `app/domain/` に
実装する。
