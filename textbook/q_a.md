...
**Q48.(Phase 9 開始時のスコープ確認)車両モデル・産業ソルバー・ジョブキューの選択**

1. **疑問が生じた Phase**: Phase 9 kickoff(「Phase9を開始する」)
2. **質問・相談内容**: README の Phase 9 節は他 Phase より簡素で、`problem_type` 文字列・スキーマ形・産業ソルバーの有無を明記していない(Phase 8 開始時の Q45 と同型の状況)。設計討議として (a) 車両モデルのスコープ(単一車両 / 複数車両 CVRP)(b) 産業ソルバートラック(OR-Tools Routing の再利用 / PuLP・scipy での MILP 化)(c) ジョブキュー(非同期実行基盤)を Phase 9 で導入するか。
3. **回答と対応方針**(ユーザーが AskUserQuestion で選択。3 点とも Claude の推奨(単一車両 / OR-Tools 再利用 / 見送り)ではなく、より野心的な側を選択):
   - **(a) 複数車両(CVRP: Capacitated Vehicle Routing Problem)**。「配送先→車両の割当」(ビンパッキング)と「車両ごとの巡回順」(TSP)の 2 段構造。README「Vehicle Model」複数形・「車両稼働率」評価指標に忠実。
   - **(b) PuLP(MILP)**。scipy は不採用。OR-Tools Routing の再利用より変更コストは高いが、README Phase 9 節「MILP 化する場合は pulp/scipy を追加」の額面どおり進める。目的関数は総距離でなく**使用台数の最小化**(古典的ビンパッキング MILP)── 経路順序そのものの MILP 化(劣周回除去制約)はスコープ外とし、割当後の巡回順は既存の TSP 近似(`optimize_waypoint_order`)に委譲(進行のルール #17 の判定基準に照らした縮小)。
   - **(c) 導入する**。README「ジョブキューを導入するとしたらここ」を実行に移す。**problem_type に依存しない横断インフラ**として追加し、既存の同期 `POST /api/v1/solve` は無変更で併存させる(既存 5 problem_type の呼び出し側は同期のまま)。ライブラリは `arq`(Redis ベース・asyncio ネイティブ)── 既存の `redis.asyncio.Redis` 共有プールとの親和性が高く、Celery/RQ のような sync-Redis 前提ライブラリを新たに増やさずに済む。
   - **この 3 点により Phase 9 は MVP 後で最大規模の拡張フェーズになった**(教材はバックエンド 8 章 + UI 1 章の計 9 章。他フェーズの平均 6〜7 章より多い)。
   - **確定した設計判断**(詳細は `Phase-9-introduction.md` および各章):
     - problem_type 名 = `logistics_planning`。道路網はノード + 区間の一般グラフ(`RouteData` に近い。travel の完全グラフに近い `legs` とは違う設計 ── README「Route Optimization」が Dijkstra/A* を挙げているのは一般グラフ前提のため)。全点対距離は Floyd-Warshall(Phase 7-1)の再利用で前処理。
     - 容量・需要は重量・体積の**2 次元**(Knapsack DP がそのまま使える。Phase 7 の予算×時間と同型)。
     - **デポ→全配送先の到達可能性は「計算」**なので `services/validation.py` が判定(`Phase-2-2.md` §3「計算か述語か」の 5 例目、複数ターゲット版)。
     - Phase 9 に**新規プリミティブはほぼ無い** ── `floyd_warshall` / `knapsack_2d` / `optimize_waypoint_order`(いずれも Phase 7)/ Branch and Bound のノード予算パターン(Phase 6)を 1 バイトも変えず再利用し、`logistics_common.py`(`travel_common.py` / `project_common.py` と同型)が糊付けする。
     - **教材の核**: `knapsack_dp` は「容量だけを見て詰める = 移動距離を無視した上界」(Phase 7「移動費用を無視した上界」/ Phase 8「資源を無視した下界」に続く 3 つ目の変奏)。容量は厳密に守るので `invalid` にはならない(travel/project との違い)── 質の劣化は `quality_ratio`(乱数プロパティ、20 seed 中 10 seed で `knapsack_dp` が `greedy` に劣る例を確認)で示す。
     - `pulp_milp` は他 4 strategy と**目的関数が違う**(総距離でなく使用台数)。fixture(D+N1-3、容量10、demand 4/5/7)では全 5 strategy が `total_distance=21・vehicles_used=2` に一致(容量の都合で {P1,P2}+{P3} が唯一の分け方)。配送先 8〜10 件規模では手実装が 3 台使うところを `pulp_milp` が 2 台に収める例をスタンドアロン検証で確認済み(教材化は見送り、本文に定性的記載のみ)。
     - **ジョブキュー**: `Job` モデルは Problem/Solution と同じハイブリッド JSONB。ワーカー(`app/worker.py`、別プロセス)は `on_startup` で専用の DB エンジン + Redis クライアントを作り、`solve_job` は `SolveService.solve` をそのまま呼ぶ(ロジック重複なし)。`_job_id` を `Job.id` と揃えて arq 側の状態と突き合わせやすくした。**Phase 9 で唯一スタブが要る章 = 9-8**(`JobService.enqueue` の unit test だけ `arq.create_pool` をフェイクに差し替え、実ワーカーの実処理は integration テスト)。
     - UI(9-9): `GraphCanvas` の車両ごとの色分けは見送り(既存 API が単色ハイライトのみ)、**small multiples**(車両ごとに 1 枚ずつ並べる)で代替(進行のルール #17 ── 拡張を今駆動する消費者が無い)。ジョブポーリングの共通部品(`api/jobs.ts` / `useJobPolling.ts`)は logistics 専用にせず `features/optimization/` 直下の feature 共通領域に配置。
