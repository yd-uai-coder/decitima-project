...
- ~~Phase 8 のトポロジカルソートは DFS ベース~~ → **Phase 15-3 で Kahn 法(入次数 BFS)へ置換済み**。DFS 版が線形依存チェーン n≈999 から `RecursionError` でクラッシュすることを実測で確認し、当時「プロジェクト完成後(Phase 15 以降)に検討する課題」としていたものが実際にトリガーされた。`has_cycle`/`successors_from_edges` は無改造、出力順は辞書順に変わった(`cpm` 等下流60テストは1件の出力順アサーション更新のみで無回帰)。経緯は `q_a.md` Q46、詳細 `Phase-15-3.md`。
...
- メタヒューリスティクス（焼きなまし法・タブーサーチ・遺伝的アルゴリズム等）による複数手法比較教材は、README §8 の手実装トラック一覧にも Phase 10〜15 計画にも含まれておらず、**将来の独立した検討課題**として扱う（着手時期未定）。現状 `optimize_waypoint_order`（Phase 4）の `_approx_best` が最近傍法+2-optという土台部分のみ実装済み（「局所最適からの脱出機構」は無し）。本格導入する場合は構成C（複数手法比較の専用教材）で新 Phase 1本相当の規模になる見込み。経緯は `q_a.md` Q49。
- **`decitima-ui` の `npm install` で `npm audit`(15件、6 moderate/8 high/1 critical)と npm 11 の `allow-scripts` 警告(`esbuild`/`better-sqlite3`/`sharp`/`unrs-resolver` 等6パッケージの install スクリプトが未承認)が出る**(Phase 15-8 の Playwright 導入作業で判明、`@playwright/test` 追加とは無関係 ── 既存の decitima-ui 依存関係に元々あったもの、`npm install` を久々に実行して初めて可視化された)。**今後の課題として対応を検討する**: `npm audit fix`(破壊的変更の可能性ありなので中身を確認してから)、`npm approve-scripts --allow-scripts-pending` での個別承認(いずれも一般的で信頼度の高いパッケージ)。着手時期未定、現状は `npm run dev`/Playwright E2E の実行自体は妨げないことを確認済み。
...
- **Phase 8（Project Manager）**: `project_scheduling` を 5 つ目の problem_type に配線（Phase 5-3 / 7-3 と同型 ── 葉 2 本 + ユニオン 1 メンバー + `semantic` 2 チェック + `structure` 1 arm + `validation.py` の閉路ゲートは **8-3**、`verification._verify_project_resources` は **8-4**（`project_common.resource_profile` 依存 ── imos）。route/network/shift/travel 無変更、`alembic` no-op。`constraints/elements.py` も無変更 ── project 解は全タスク実施で forbidden/required 非該当）/ **依存 DAG の閉路検出は「計算」** ── `algorithms/graph/topological.py::has_cycle` を `services/validation.py` が呼ぶ（`InfeasibleProblemError`）。`Phase-2-2.md` §3「計算か述語か」の **4 例目**（route 到達可能性 / network 連結性に続く）。`ProjectData.model_validator` は端点実在・id 一意・自己依存禁止のみ / `graph/topological.py` = **DFS 後行順の反転 + gray/black で back edge = 閉路**（README「Phase 1 の DFS が土台」）。出力は「辞書順」でない。理論章で Kahn 法（入次数 BFS）を対比。registry 非搭載 / `scheduling/critical_path.py::cpm` = **前進パス（ES/EF）→ makespan → 後退パス（LS/LF）→ slack → クリティカルパス復元**（generic dict を取る ── `ProjectData`（8-3）に前方依存しない設計。`floyd_warshall` / `knapsack_2d` と同じ）。クリティカルパスは複数あるとき単一（後続 id 昇順） / `duration: int` に限定（imos グリッド。README「完了予定日」も日単位）。時刻は t=0 起点、日付は UI 側 / **教材の核 ── cpm は依存だけを見て資源を無視 =「資源を無視した下界」**（Phase 7 の「DP は移動費用を無視した上界」と対）。capacity 超過で Verification が `invalid`。`priority_list`（LS 昇順の貪欲 SGS）= 資源 feasible だが最適でないことがある。`cp_sat`（OR-Tools、interval var + `add_cumulative` + `minimize(makespan)`、決定論 = `num_search_workers=1` + `random_seed=0`）= 厳密 RCPSP。fixture では **cpm=8（invalid）/ priority_list=10 / cp_sat=9**。`cpm ≤ cp_sat ≤ priority_list` / `cpm_nx`（networkx、`nx.topological_sort` + 手実装と同じ CPM 算術）= 非制約 CPM のオラクル（`critical_chain` は 8-2 の公開関数を再利用） / **Difference Array（Phase 6 `range_add`）を 1 バイトも変えず再利用**（2 人目の消費者 = `resource_profile`。`on_duty_by_hour` と同じ形） / `family="scheduling"` 再利用 / objectives = `makespan` + `peak_resource`（minimize） / `select_strategy`: 資源制約あり → `priority_list` / なし → `cpm`。厳密は `?algorithm=cp_sat` / **章の帰属 ── 8-1: `topological`（純粋）/ 8-2: `critical_path`（純粋、`topological` にのみ依存）/ 8-3: ドメイン配線 + 閉路ゲート（fixture 含む）/ 8-4: `project_common` + `cpm` + `priority_list` + `_verify_project_resources` / 8-5: `ortools_project` / 8-6: `networkx_project` + registry + select + e2e / 8-7: UI + `GanttCanvas`** / UI は 5 スライス目、`GanttCanvas`（`components/ui/charts/`、`{id,label,start,end,slack?,highlight?}[]`）は新規・ドメイン非依存（テンプレート還元候補）、依存 DAG は `GraphCanvas` 再利用。analysis トラックは後続 Phase 送り。詳細は Q45。
- **Phase 9（Logistics Optimizer ── MVP 後 3 つ目の拡張フェーズ）**: `logistics_planning` を 6 つ目の problem_type に配線(CVRP。複数車両・容量制約。ユーザーへの kickoff 確認で「複数車両」「PuLP MILP」「ジョブキュー導入」を選択 ── 詳細 Q48)。葉 2 本(`LogisticsNode`/`RoadSegment`/`Vehicle`/`DeliveryStop`)+ ユニオン 1 メンバー + `semantic` 3 チェック + `structure` 1 arm + `validation.py` の到達可能性ゲートは **9-1**、`verification._verify_logistics_routes` は **9-2**(`logistics_common.capacity_ok`/`route_distance` 依存)。route/network/shift/travel/project 無変更、`alembic` は初めて **実テーブル `jobs` が増える**(9-8)/ **デポ→全配送先の到達可能性は「計算」** ── `logistics_deliveries_reachable`(複数ターゲット版)。`Phase-2-2.md` §3「計算か述語か」の **5 例目**(route 到達可能性 / network 連結性 / project 非巡回性に続く) / **Phase 9 に新規プリミティブがほぼ無い** ── `floyd_warshall`・`knapsack_2d`・`optimize_waypoint_order`(いずれも Phase 7)・B&B のノード予算パターン(Phase 6)を**1 バイトも変えず**再利用、`logistics_common.py`(`travel_common.py`/`project_common.py` と同型)が糊付けするだけ / **教材の核 ── `knapsack_dp` は容量だけを見て詰める =「移動距離を無視した上界」**(Phase 7「DP は移動費用を無視した上界」・Phase 8「cpm は資源を無視した下界」に続く 3 つ目の変奏)。容量は厳密に守るので travel/project と違い `invalid` にはならない ── 質の劣化は `quality_ratio`(乱数プロパティ、20 seed 中 10 seed で `knapsack_dp` が `greedy` に劣る例を確認済み)。`greedy` は 1 件ずつ実距離増分で判定 → 必ず valid。`branch_and_bound` の下界は「確定距離」(三角不等式により単調非減少 ── 安全だが緩い、Phase 6 の楽観推定より弱い)。`brute_force` が正解オラクル(小規模)/ **`pulp_milp`(PuLP+CBC)は他 4 strategy と目的関数が違う** ── 総距離でなく**使用台数を最小化するビンパッキング MILP**(README §12.5「車両稼働率」に対応)。経路順序そのものの MILP 化(劣周回除去制約)はスコープ外 ── 割当後の巡回順は既存 TSP 近似に委譲(進行のルール #17)。fixture(D+N1-3、容量10、demand 4/5/7)では手実装 4 本 + PuLP が全て `total_distance=21・vehicles_used=2` に一致(容量の都合で {P1,P2}+{P3} が唯一の分け方)。配送先 8〜10 件規模では `knapsack_dp`/`greedy` が 3 台使うところを `pulp_milp` が 2 台に収める例を確認済み(教材化は見送り、chapter 本文に定性的記載のみ) / **ジョブキュー(9-8)= problem_type に依存しない横断インフラとして新規導入**(MVP 以来の YAGNI をここで解除)。`arq`(Redis ベース・asyncio ネイティブ、既存の `redis.asyncio.Redis` 共有プールと親和)を選定、Celery/RQ のような sync-Redis 前提ライブラリは増やさない。`POST /api/v1/jobs` / `GET /api/v1/jobs/{id}` を新設し既存の同期 `POST /solve` とは**無変更で併存**。`Job` モデルは Problem/Solution と同じハイブリッド JSONB。ワーカー(`app/worker.py`、別プロセス)は `on_startup` で専用の DB エンジン + Redis クライアントを作り、`solve_job` は `SolveService.solve` をそのまま呼ぶ(ロジック重複なし)。`_job_id` を `Job.id` と揃えて arq 側の状態と突き合わせやすくした / **Phase 9 で唯一スタブが要る章 = 9-8**(`JobService.enqueue` の unit test だけ `arq.create_pool` をフェイクに差し替え、実ワーカーの実処理は integration テスト)── 進行のルール #14 の実例 / `family="optimization"` 再利用 / UI は 6 スライス目、`GraphCanvas` の車両ごとの色分けは**見送り**(既存 API が単色ハイライトのみのため)、代わりに **small multiples**(車両ごとに 1 枚ずつ並べる)で対応 ── 進行のルール #17 の判断(色分け機能を拡張する実在の消費者が無い)。ジョブポーリングの共通コンポーネント(`api/jobs.ts`/`useJobPolling.ts`)は logistics 専用にせず `features/optimization/` 直下の feature 共通領域に配置。`useJobPolling` はフェイクタイマー環境で `@testing-library/react` の `waitFor` がデッドロックする落とし穴があり `vi.advanceTimersByTimeAsync` を `act()` で包んで代替(教材に明記)。detail は Q48。

- **Phase 10（What-if Simulation ── MVP 後 4 つ目の拡張フェーズ）**: 新しい `problem_type` やドメインアルゴリズムを増やさず、Phase 4〜9 の6ドメインを横断する**意思決定支援層**として実装(README §13/§19。開始前のユーザー確認 3 点は Q53)。①**実行モデルは非同期**(Phase 9-8 の `jobs` テーブル/arq を再利用。新テーブルなし。実消費者 = 複数シナリオ×重いアルゴリズムを想定した進行のルール #17 判定)②**override は汎用 dict マージ**(`apply_overrides`。RFC 7386 JSON Merge Patch 相当 + Pydantic 再検証。ドメイン別の override コードは 0)③**Sensitivity Analysis に二分探索(`find_threshold`)を追加** ── Phase 1 `binary_search`(値の探索)とは別の応用(答えを二分探索する)、全件スイープ O(n) との複雑さの対比が教材の核 / `SimulationService`(`run_simulation`)は Phase 3 `BenchmarkService` と対称(Benchmark=同じ問題×違うアルゴリズム、Simulation=同じアルゴリズム×違う問題)だが**永続化を持たない**(session/redis 依存なしの素の async 関数、Phase 10 で初めて) / **Phase 9 への改訂 1 件**:`JobStatusResponse.result` を `CandidateSolution | SimulationResult | None` に広げた。両型の必須フィールドが重ならないため discriminator タグ不要(Pydantic smart union)、`solve_job` の実装・既存 e2e テストは無改造(#12.4 のスモークが最小コストで済む好例、詳細 `Phase-10-4.md`)/ `find_threshold` の対象は list に入っていない top-level スカラー限定(`ProjectData.resource_capacity` が具体例。merge patch が list を丸ごと置換するため要素単位の override に向かない制約) / UI はシナリオ入力を `ProblemJsonEditor` 同型の JSON テキストエディタに統一(ドメイン別シナリオビルダーは見送り)、`useJobPolling`(9-9)を無変更で再利用。

- **Phase 11（LLM Problem Structuring ── LLM が初めて登場するフェーズ）**: README §20「自然言語 → LLM → Structured Problem → Validation」。開始前のユーザー確認 4 点は Q59(LLM は既存 Gemini 継続 / 既存 Web検索QAチャットワークフローは全面作り替え / 6ドメイン全部に最初から対応 / Human-in-the-loop 確認UIまで作る)。**最大の設計論点 ── グラフ構造ドメイン(route/network/project/logistics)の非対称性**: ノード/エッジ/タスクのカタログを自然言語1文から LLM に発明させるのは非現実的。**「LLM が埋めてよいのは objectives/constraints/data のトップレベル・スカラーまで、カタログは常にベース問題(`app/domain/problems/base_problems.py`、6ドメイン分・新規)から引き継ぐ」という1原則を6ドメイン共通に適用**し、機構は Phase 10 `apply_overrides` を**第二の消費者**として再利用(`base_problem + LLM抽出パッチ → apply_overrides → Validation`)。`EXTRACTORS`(`app/algorithms/registry.py::REGISTRY` と同型のディスパッチテーブル)は `network_design` のようにトップレベル・スカラーを持たないドメインで自然に LLM 呼び出しをスキップする(グラフの分岐エッジではなくノード内部のレジストリ参照で吸収)。**グラウンディング検査(`ground_references`)を新設** ── 既存 `ProblemValidationService` は `RequiredInclusionConstraint.items` 等の id 参照が実在するかを検査しない(到達可能性の計算にしか使わない)ため、LLM が名前(「浅草」)を id の代わりに出力するハルシネーションを検知する最後の砦として追加(新しい例外クラスは増やさず既存 `ProblemValidationError` を再利用)。LangGraph ワークフローは `classify_problem_type → load_base_problem → extract_objectives_constraints → extract_domain_data → assemble_problem → validate_problem` の一直線パイプライン(条件分岐エッジ無し)。**既存(テンプレート由来)の Web検索QAチャットワークフロー(Gemini+LangGraph+Tavily)を全面廃止** ── `app/ai/tools/tavily.py`・`app/services/chat.py`・`app/api/routes/chat.py`・`app/schemas/generation.py` を削除(`Conversation`/`Message` モデル・`ConversationRepository` は無改造のまま Phase 11 で初めて実消費者を得る)。`app/ai/graph/nodes.py` が `app/services/structuring.py` の関数を import し、後者が前者の `get_structuring_workflow` を呼ぶ設計は循環 import を生むため、後者は関数内 import で輪を断つ。`with_structured_output().invoke()` の戻り型は `dict | BaseModel` にしか narrowing されないため `cast()` で明示 ── これにより `app/ai/**` の既知の型債務(pyright ignore)を解消した。`POST /api/v1/structure` は新設するが「確定して solve」専用エンドポイントは作らず、返る `problem` をそのまま `POST /api/v1/solve` に渡す設計(README「LLM Service はスキーマ経由でのみ Algorithm Engine と接続する」)。UI も新しい solve ビューアを作らず、共有 `pending-problem-store`/`usePendingProblemHydration` で既存6ドメインページへ遷移する(`useJobPolling` と同じ「problem_type に依存しない共通フック」の設計方針)。backend 576 passed / ui 64 passed。detail は Q59。
- **Phase 12（Algorithm Recommendation）**: README §9「Algorithm Selection」の3段階セレクション(Step1 ルールベース／Step2 LLM推薦／Step3 ベンチマークベース)の第2段。キックオフ確認3点は Q61(出力形状=候補リスト+理由を返す新エンドポイント／発火タイミング=明示的な opt-in／対象ドメイン=Phase 11 同様6ドメイン全部)。**既存 `/solve`/`/benchmark` の既定選択(`select_strategy`)には一切触れず、完全に追加的なレイヤーとして載せる**(README「LLM単独では最終決定しない」の徹底)。**LangGraph は使わない** ── rule計算1回+LLM呼び出し高々1回の単純な流れのため、Phase 9/10 の `SimulationService`/`BenchmarkService` と同じ判断で素の async 関数(`AlgorithmRecommendationService`)にした。`_ALGORITHM_DESCRIPTIONS`(`_PROBLEM_TYPE_DESCRIPTIONS` と同型の静的テーブル)は **`(problem_type, meta.name)` のタプルをキーにする** ── `meta.name` は "greedy"(shift/travel/logistics の3実装)・"brute_force"(route/travel/logistics の3実装)のように problem_type をまたいで重複するため、`name` 単体キーだと異なる実装の説明が上書きし合う実バグになる(設計検討中に発見・修正)。LLM のグラウンディング(存在しない候補名を無視)は Phase 11 の `ground_references` と同じ原則だが、対応の強さは変える ── structuring は `ProblemValidationError` を送出する(下流の `/solve` に影響するため)のに対し、recommend は該当候補を除外して notes に記録するだけ(推薦は表示専用の補助情報でパイプラインを止める理由が無いため)。LLM 呼び出し失敗も同じ理由でハードエラーにせず、ルールのみへグレースフルデグレードする(ただし Phase 11-8 の教訓通り `logger.warning` は必ず残す)。ルートは新設せず既存 `routes/algorithms.py` に `POST /recommend` を追記(「algorithms という同じ操作対象」の判断、CLAUDE.md の「操作で割る」既存方針に従う)。`tests/fixtures/fake_llm.py`(Phase 11-3)に `ainvoke` を第二の消費者として追加(Structuring の LangGraph ノードは同期 `invoke` を `run_in_executor` 任せにできたが、Phase 12 は LangGraph を介さないため自分で非同期呼び出しする必要があった。既存 `invoke` は無変更)。UI は `features/optimization/{api,stores,hooks,components}` に推薦カードを追加、6 Planner Panel へ1行ずつ導線を配線(`useJobPolling`/`pending-problem-store` と同じ「problem_type に依存しない共通領域」)。backend 510 passed / ui vitest 新規5 passed(既存164無回帰)。detail は Q61。
- **Phase 13（Result Explanation）**: README §13 は目的・パイプライン図・説明対象5項目(なぜこの解か／どの制約が重要か／どのアルゴリズムか／他候補との違い／改善余地)・設計ポイント2行のみの薄い定義。キックオフ確認3点は Q62(対象データ=永続化済み `Solution` を id 指定／他候補との違い=Phase 12 の静的説明表を比較材料にする／永続化=しない)。**新エンドポイントは `POST /api/v1/solutions/{solution_id}/explain`** ── 既存 `routes/solutions.py` に追記(「solutions という同じ操作対象」、Phase 12 の `routes/algorithms.py` への追記と同型)、既存 `OptimizationReadService.get_solution`/`get_problem`(Phase 1、所有者スコープ付き)を第三の消費者として無変更で再利用。**「他候補との違い」は他アルゴリズムを再 solve しない** ── Phase 12 の `_ALGORITHM_DESCRIPTIONS` を `problem_type` でフィルタするだけで比較材料が揃う(registry への依存なし、Phase 12 より単純)。この帰結として `_ALGORITHM_DESCRIPTIONS`(Phase 12 では `algorithm_recommendation.py` 内の非公開定数)が Phase 13 で2人目の消費者を得たため、`app/domain/problems/algorithm_catalog.py` へ抽出し `ALGORITHM_DESCRIPTIONS`/`describe_algorithm`(公開)に改名した(進行のルール #17「今駆動している実在の消費者」に Phase 13 が具体名で答えられるケース、Phase 12 コードへの遡及変更は進行のルール #12 で記録 ── `Phase-12-introduction.md` §11)。**LLM 失敗時のフォールバック設計は Phase 12 と異なる** ── 推薦は「ルールのみで返す」がそれ自体価値を持つのに対し、Result Explanation は「narrate すること」自体が価値のため、失敗時は `metrics`/`violations` を直接文字列化した機械的な要約にフォールバックする(`logger.warning` は Phase 11-8/12 の教訓通り必ず残す)。**LangGraph は使わない**(Phase 12 と同じ判断、DB読み取り1回+LLM呼び出し高々1回)。UI 側は各 Planner Panel の既存「解く」が `persist: false`(探索的な試行を DB に残さない既存方針)のため explain がそのまま呼べない ── `ExplanationCard` 専用の永続化つき `persistSolve`(`features/optimization/api/solve.ts`、新規・別ファイル)を用意し、押下時に solve(persist: true)→ explain の2段階を store 内部で完結させることで既存「解く」ボタンの挙動には一切触れずに解決した。backend `uv run pytest` 602 passed(新規14) / ui vitest 77 passed(既存回帰なし)。detail は Q62。
- **Phase 14（LLM vs Algorithm Comparison ── LLM 層の最後、README「プロジェクトの核心的な検証テーマ」）**: README §14 は目的・比較図・評価軸6項目(制約遵守率・最適性・再現性・実行時間・エラー率・検証可能性)のみの薄い定義。キックオフ確認4点は Q63(対象=6ドメイン全部／LLM Only の実装方式=`AlgorithmStrategy` 準拠ラッパー(REGISTRY 非登録)／比較する Algorithm=既定選択1本のみ／再現性測定と永続化=LLM を N 回再実行+非永続)。**設計を単純化した発見 ── 既存 `SolutionVerificationService` の構造検証(6ドメイン全て)は申告された派生値(total_weight 等)を常に生の構造から再計算し、食い違えば hard violation にする**。これにより LLM に既存の `RouteSolution`/`ShiftSolution`/…と全く同じスキーマを `with_structured_output()` で出力させれば、変換コード無しで既存 Verification にそのまま通せる ── 新しい「LLM 用解スキーマ」が不要になり、実装量が大きく減った(手実装 Algorithm と全く同じ検証コードで LLM 解を検証するため比較が公平になる、という「検証可能性」の実演にもなる)。**`AlgorithmMeta.family` に `"llm"` を追加**(Phase 1 `domain/solutions/solution.py` への遡及、6つ目のサブパッケージ `app/algorithms/llm/` と対応)、`LlmOnly*Strategy` 6本は本番 `REGISTRY` には登録せず `ComparisonService` 専用の辞書から直接インスタンス化 ── `/solve` の既定選択には一切影響しない(Phase 12 の判断を踏襲)。**`ComparisonService` は Phase 3 `measure_call` を再利用しない** ── LLM Only の非決定論性が「複数回実行して代表値1つに潰す」という `measure_call` の前提と相容れないため、`_run_once` を自前で書き `llm_runs` 回ループする(Algorithm 側は決定論的なので1回で十分。この非対称自体が「再現性」を測る教材の核)。**「測定」と「その説明」で信頼性要件を変える** ── LLM Only の実行(6軸の元データ)にはリトライもフォールバックも無し(生の信頼性を測るため失敗を隠さない)、比較結果のナレーション生成には Phase 12/13 と同じグレースフルデグレードを適用(narrate すること自体が価値なため)。**`_objective_metrics` が本 Phase 唯一の非自明な発見** ── 目的の対象値(total_weight 等)は route/network/travel/project/logistics では `assignments` 側のフィールドに、shift だけは `metrics`(`assignment_metrics()` 経由)にあり、この非対称は Phase 14 で「全ドメイン共通で目的値を1箇所から引く」ことが要求されて初めて顕在化した。再現性は解の構造を要約したハッシュ(`structure_hash`)で「何種類の構造が出たか」を数え、妥当性とは独立に測る。エンドポイントは新規 `routes/comparison.py`(`POST /api/v1/compare`、Benchmark とは対象が異なるため既存 `routes/benchmark.py` には追記しない、Phase 9/11 と同じ「新しい操作対象には新ファイル」判断)、永続化なし(Phase 9 Simulation・Phase 12 Recommendation と同型)。UI は `AlgorithmRecommendationCard`(Phase 12)・`ExplanationCard`(Phase 13)に続く3つ目の横断コンポーネント `ComparisonCard`、`/compare` が素の `problem` を受けるため Phase 13 のような「先に persist solve」の2段階は不要(`/recommend` と同型の1段階)。**実装完了後、実際に本物の Gemini で `/compare` を叩いたところ6ドメイン全滅で発覚した罠 ── `RouteSolution` 等の `problem_type: Literal[...] = "..."`(判別子)をそのまま LLM に見せると、Gemini の構造化出力(`response_json_schema`)が Pydantic の `"const"` 制約(単一値 Literal の JSON Schema 表現)をサポートせず無視するため、LLM が無関係な値を生成し `literal_error` で全滅する**。`FakeLLM` のユニットテストは固定値をそのまま返すだけなので発見できず、本物の LLM 呼び出しで初めて顕在化した。対処: `app/algorithms/llm/common.py::strip_problem_type()`(`create_model` で判別子を除いた派生スキーマを動的生成)を追加し、`solve()` 側で固定値を足し戻す ── 既存6スキーマをそのまま使うという設計判断自体は変えていない。backend `uv run pytest` 621 passed(新規19、既存602件は無改造で再実行し回帰なし) / ui `npx vitest run` 新規6件 passed(既存回帰なし、`Menu.test.tsx` の pre-existing 失敗1件は無関係)。detail は Q63。
- **Phase 15（Production ── 最終フェーズ）**: README §15 は Testing/Performance/Security/Deployment の4本柱のみの薄い定義、「テンプレートが既に足場を提供しており作り直さない」が設計のポイント。開始前のユーザー確認4点(デプロイは教材化のみ・E2Eは代表2ドメイン(Route Planner+Travel Planner)・負荷テストツール(locust/k6)は見送り・4本柱すべてカバー)。**この Phase の核心は「実測してから直す」手順そのもの** ── 結論は緩和・現状維持・実際の不具合修正・見落とし発見の4通りに分かれた。**`_MAX_KNAPSACK_DP_CELLS`(Phase 11-9)の実測で、ガードが実際に保護しているのは `POST /solve` の素の1回呼び出しだけで `POST /benchmark`(`get_strategies` で全候補を回す)は対象外という前提誤りを発見**、さらに `measure_call`(`tracemalloc` 計装)が大量の小オブジェクト割当アルゴリズムで10倍以上の見かけの遅さを生むことも判明(knapsack_2dで1.84秒→20.6秒)。`POST /solve` の実経路基準で再計測し閾値を2,000,000→4,000,000に緩和、`logistics_planning` にはガード不要(`LogisticsDataPatch` が `depot_id` しか公開せず LLM が capacity に触れないため攻撃面が無い)と判断。**`topological_sort`(DFS、Phase 8-1)は線形依存チェーン n≈999 から実際に `RecursionError` でクラッシュする**ことを実測で確認、Phase 8-1 が予告していたKahn法(入次数キュー、反復)へ置換 ── `has_cycle`/`successors_from_edges` は無改造、出力順は辞書順に変わったが `cpm` 含む下流60テストは1件のアサーション更新のみで無回帰。DBクエリ(`get_solution`/`list_solutions_for_problem`)は実運用よりかなり大きい合成データ(20,000問題、1問題に5,000解)でも既存インデックスだけで数ミリ秒 ── 追加インデックス不要と判断。**CPUバウンドな arq ジョブは同時実行数を増やしても真の並列化はされない**(CPython の GIL、10並列で単発の約10.4倍)ことを実測し `WorkerSettings.max_jobs` を10→4に。**`SolutionExplanationService.explain()`(Phase 13)を Phase 15 で唯一の実在するキャッシュ消費者に選定** ── `BASE_PROBLEMS`/`ALGORITHM_DESCRIPTIONS` は純メモリ辞書でI/Oコストが無くルール#17の「実在の消費者」テストに落ちる一方、`explain()` は不変な `Solution` に対し Gemini API への実コストある呼び出しを繰り返していたため。所有者チェックは必ずキャッシュ参照より先、フォールバック応答はキャッシュしない設計。**セキュリティ監査(6項目)は5項目が既存資産で対応不要、`.env.example` に Phase 11 で廃止した `TAVILY_API_KEY=` の消し忘れのみ発見**。**E2E(Playwright)を初導入** ── Route Planner(直接入力パターン)+ Travel Planner(LLM構造化パターン)の2ドメインで6ドメイン共有の2入力パターンを代表させる。実測で2つのバグを発見・修正: ログイン後の `router.replace` 完了待ちが必要なタイミング問題、開発サーバーのポート番号を `CORS_ORIGINS` 既定値(3000番)に合わせる必要があること(Phase 15-7 のCORS監査結果がE2Eで実演された)。**E2E は pytest の `monkeypatch` が使えない**(Playwright は本物のサーバープロセスに外部からアクセスするため)ので `get_gemini_llm()` 自身が `settings.E2E_TESTING` を見てフェイク(`E2eFakeLLM`)を返す設計にし、8箇所の呼び出し元は無改造で済んだ。フェイクは実際にシナリオが踏む3スキーマのみ対応し未対応は `NotImplementedError`(進行のルール#17)。実測で `invoke`(同期、`nodes.py`)と `ainvoke`(非同期、`explanation.py`等)の両対応が必要と判明(`ainvoke`のみの実装で `POST /structure` が502で落ちた)。**`analysis/benchmark_report.py::regression()`(Phase 3-8 で「Phase 15 のCI回帰検知用」として先行実装)を初めて配線** ── 代表3アルゴリズムを `measure_call` で実測しコミット済みベースラインと比較、閾値(3倍)超過でCI失敗。**本番 `docker-compose.prod.yml` に `worker`(arq)サービスが無い見落としを発見**(開発用 compose にはPhase 9-8で追加済み)── `POST /jobs`/`POST /simulate` が受理されるが処理されず無応答になる実害のあるギャップ、追加して解消。CI/CD(`.github/workflows/{backend-ci,ui-ci}.yml`)・デプロイRunbook(`DEPLOYMENT.md`)・セキュリティ監査(`SECURITY.md`)を新設。backend `uv run pytest` 636 passed(既存回帰なし)/ `uv run pytest -m performance` 全数green / `ruff`/`uvx pyright` 0件(`app/services/errors.py` のpre-existing債務は対象外)。detail は `textbook/q_a.md` 該当エントリ。
...
- **`pulp` / `arq` を実 `.venv` に導入した(Phase 9)** — samples 検証時、共有 `.venv`(overlay がシンボリックリンク)に `uv pip install 'pulp>=2.9' 'arq>=0.26'` を実行済み。`arq` の依存解決で `redis` が `8.1.0` → `5.3.1` に**ダウングレード**された(プロジェクトの制約 `redis>=5.0` は満たすので既存コードへの実害は無し、`hiredis` が新規追加)。`pyproject.toml` / `uv.lock` はユーザーが `uv sync` するまで未更新 ── 実行すると `arq` の制約次第で同じダウングレードが起きる可能性があるので、コンフリクトが出たら `redis` のバージョン範囲を確認すること。`decitima-api` / `decitima-ui` 本体リポジトリへの overlay は必ず `git archive HEAD | tar -x -C <一時ディレクトリ>` でクリーンな複製を作ってから行う(実リポジトリを直接 rsync で上書きしない ── Phase 9 検証中に一度誤って直接上書きし、`git checkout -- . && git clean -fd` で復旧した)。
...
- **isinstance ディスパッチ arm の番人テストは「arm 未接続で赤」でなければ #15 の穴(Q44)** — ユーザーの `structural_verify` 写経が network(5-3)/ travel(7-3)の 2 arm を欠き、`verify_*_structure` が dead code 化 → travel の予算超過 hard チェックが走らず `test_dp_can_overrun`(7-5)で初めて FAIL。原因: `test_structural_verify_dispatches_{network,travel}` が `assert isinstance(violations, list)` / `assert violations == []`(良い解)しか見ておらず、ルーティングが切れても緑。対策: 両テストを「ルーティング先の violation をアサート」に強化(arm を外すとその章のテストが即赤)。教訓 = 章の配線を章のテストがその場で突く(Q30 / Q38 と同型)。Phase 8-3 の `test_structural_verify_dispatches_project` は最初からこの形(`finish != start + dur` の解 → `project_structure` violation。overlay で逆確認済み)。
- **Phase 8 ── DFS トポロジカルソートの出力は「辞書順」ではない** — 後行順の反転なので、先に深く潜った近傍ほど末尾に回る(`A -> {B, C}` は `[A, C, B]`)。Kahn 法(ソート済みキュー)なら `[A, B, C]`。8-1 のテスト `test_deterministic_neighbour_order` は実出力 `["A", "C", "B"]` をアサートし、これを教材の対比点にしている。`_respects` ヘルパで「全辺が守られているか」も別途確認。
- **Phase 8 ── `NetworkxCpmStrategy` の後退パスは `LF[t] = min(LS[s] for t→s)`** — 前進パスの `ES` でなく後続の `LS` を使う。生成時に `lf[p] = min(lf[p], es[t])` と書き誤り、`test_cpm_and_cpm_nx_agree`(ランダム DAG 6 seed)で `slack` / `critical_path` が手実装とズレて発覚 → 手実装 `cpm` と同じ算術(`succ_ls = [ls[s] for s in successors]; lf[t] = min(succ_ls) if succ_ls else makespan`)に修正。教材(8-6 §1)は前進 / 後退パスを両方見せる。**別実装オラクルはランダム入力で手実装と突き合わせて初めて写経ミスが出る**(Phase 4 の `test_networkx_matches_handwritten_dijkstra_property` と同型)。
- **Phase 9 ── `logistics_common.route_for_vehicle` の depot 抽出は `node_order[1:-1]`(先頭と末尾を両方削る)が正しい** — `optimize_waypoint_order(depot, depot, ...)` の戻り値は必ず `[depot, ...訪問順..., depot]` の閉路。生成時に travel の `order_and_cost`(`tour[:-1]` ── 末尾だけ削る。travel は起点 place 自体を訪問先として含める設計のため正しい)を素朴に真似ると、logistics では先頭に depot が residual で残る。教材生成中にスタンドアロン検証(`uv run --python 3.13` の使い捨てスクリプトで `knapsack_dp`/`greedy`/`brute_force`/`branch_and_bound` を fixture の期待値 `total_distance=21` と突き合わせ)で発覚し出荷前に修正 ── ユーザー写経後の手戻りを防げた。**教訓 = 「似ているが travel とは前提が違う」箇所(depot がノード集合に含まれるか否か)は必ず数値で突き合わせる**、Phase 4 `test_networkx_matches_handwritten_dijkstra_property` / Phase 8 `NetworkxCpmStrategy` と同型の「別経路での検算」。
- **Phase 9-9 ── `useJobPolling` は `react-hooks/set-state-in-effect` に抵触しやすい** — 「jobId が変わったら job/error をリセットする」を effect 冒頭で同期的に `setState` すると lint error(cascading render 警告)。React 公式の「prop の変化に応じて state を調整する」パターン(レンダー中に `if (jobId !== trackedJobId) { setTrackedJobId(jobId); setJob(null); ... }` と比較して直す)に置き換えて解消。あわせて `@testing-library/react` の `waitFor` はフェイクタイマー環境で実タイマーのポーリングに依存しデッドロックする ── `vi.advanceTimersByTimeAsync` を `act()` で包んで代用(教材 `Phase-9-9.md` に明記)。
- **Phase 9-6 ── `pulp.PULP_CBC_CMD` → `pulp.COIN_CMD` への移行は現行 pip 配布版(3.3.2)ではまだ機能しない** — PuLP は `LpVariable` 直接生成と `PULP_CBC_CMD` の 2 つに `DeprecationWarning` を出すが、前者(→ `prob.add_variable(...)`)は安全に解決できる一方、後者は罠。Context7(PuLP 公式 GitHub)は「`pulp[cbc]` を入れれば `COIN_CMD` が同梱 CBC バイナリを自動検出する」と説明するが、実機検証(pip 配布版 3.3.2)では `COIN_CMD().path` は文字列 `"cbc"` 固定(PATH 頼み)で `pulp[cbc]` を入れても自動では拾わず、切り替えると `PulpSolverError: cannot execute cbc` で全滅する(ユーザーの写経先でも同じエラーを再現)。`pulp.LpSolverDefault` も中身は `PULP_CBC_CMD` のままで、PuLP 自身が内部移行を完了していない。**教訓 = ドキュメント(特に GitHub 最新)が pip 版の実際の挙動より先行していることがある。ライブラリの deprecation 警告は Context7 で調べた移行先をそのまま採用せず、実機で解決を確認してから反映する**。結論: `add_variable` のみ反映し `PULP_CBC_CMD` は維持(警告は実害無しとして許容)。詳細 `q_a.md` Q50。
- **Phase 9-8 ── `test_jobs_e2e.py` の最終検証が identity map に阻まれて失敗する** — `pg_session`(`expire_on_commit=False`)で `job` を作った後、`solve_job` が**別セッション**で status を更新・commit しても、`pg_session` で `get_by_id` すると `AsyncSession.get()` が identity map の古いインスタンス(status="queued")をそのまま返し DB に再クエリしない。本番の `GET /jobs/{id}` はリクエストごとに新規セッションなのでこの穴は無い ── バグはテストが `pg_session` を enqueue と最終検証の両方で使い回したことのみ。修正: 最終検証だけ新しいセッションで読み直す。詳細 `q_a.md` Q52。
- **Phase 10 ── `ruff`(flake8-async ASYNC109)は `async def` の引数名 `timeout` を警告する** — 「`asyncio.timeout()` コンテキストマネージャを使うべき」という指摘。挙動は変えず引数名を既存 `SolveRequest.timeout_seconds` に揃える(`timeout_seconds`)だけで解消。今後 `async def` に時間の上限を渡す引数を追加する際は最初から `timeout_seconds` にする。
- **Phase 10-4 ── 型が重ならない union は discriminator タグ無しで Pydantic の smart union に判別を任せられる** — `JobStatusResponse.result` を `CandidateSolution | SimulationResult | None` に広げる際、両モデルの必須フィールドが1つも重ならないことを確認した上で素の union のまま広げた(`kind` タグや値のラップ無し)。既存の書き込み側(`solve_job`)・既存テストのアサーションが無改造で済む最小の改訂になった ── 後続 Phase で同種の型拡張をする際、まず「必須フィールドが重なるか」を確認する。
- **Phase 10-2 ── 章タグの後退境界に再タグが無く `run_simulation` が写経漏れになった** — samples `app/services/simulation.py` で `# (Phase 10-3)` の `_run_sensitivity` 直後に続く `run_simulation`(10-2 の内容)に再タグが無く、直前のタグを引き継いで 10-3 の一部に見えた。対策: `run_simulation` に `# (Phase 10-2)` を追加、#12.2 に「章番号が後退する境界は必ず再タグ」を明記(Q56)。
- **Phase 10-4 ── `test_jobs_e2e.py` に2本目のテストが加わり `attached to a different loop` が顕在化** — 共有 `engine`(`pool_pre_ping=True`、プロセス全体で1つ)を、テスト関数ごとに新しい event loop を作る pytest-asyncio の下で使い回すと、前のテストの接続を次のテストが別 loop で ping しようとして落ちる。対策: `pg_session` の末尾に `engine.dispose()` を追加(Q57)。
- **Phase 9-8/10-4 ── worker の simulate ジョブが `queued` のまま無応答になることがある** — `app/worker.py::on_startup` の専用エンジンに `pool_pre_ping=True` が無く、プール内の死んだ接続をそのまま使うと `InterfaceError: connection is closed` が try/except の外(最初の `get_by_id`)で発生し Job 行が `running` にすら更新されずクラッシュする。対策: `on_startup` に `pool_pre_ping=True` を追加(Q58)。
- **Phase 11 ── `app/ai/graph/nodes.py` と `app/services/structuring.py` が互いを import して循環する** — nodes.py は structuring.py の `EXTRACTORS`/`catalog_entries` 等を、`ProblemStructuringService.structure()` は `app/ai/graph/workflow.py::get_structuring_workflow` を呼ぶ。モジュール先頭 import だと輪になるため、後者を `structure()` メソッド内の関数内 import にして断つ(実行時コストは import キャッシュで無視できる)。
- **Phase 11 ── overlay で `app/ai` の pyright ignore を外すと隠れていた型エラーが噴出する** — `with_structured_output().invoke()` の戻り型は `dict | BaseModel` にしか narrowing されず、`result: SpecificSchema = llm.invoke(...)` は `reportAssignmentType`。`cast(SpecificSchema, ...)` で明示的に narrowing する。ignore リストに入っている間は個別ファイル指定の `uvx pyright` でも診断が抑制されるため、旧テンプレートコードはこの型債務を隠したまま存在していた。
- **Phase 11 ── Web検索QA機能の廃止に伴う3ファイル削除は rsync ベースの overlay では消えない** — `app/ai/tools/tavily.py`・`app/services/chat.py`・`app/api/routes/chat.py` は他 Phase に無い「既存ファイルの削除」を伴う初めてのケース。`textbook/samples/README.md` の overlay 手順に `rm` を明記(実リポジトリでも同じ3ファイルをユーザー側で削除する)。
- **Phase 11-7 ── `config.py` への `STRUCTURE_RATE_LIMIT_PER_HOUR`/`_PER_DAY` 追記漏れ(ユーザー写経で発覚)** — 新規ファイル `structuring.py`(`RateLimiter` 構築で両設定を参照)は写経済みだったが、それが依存する既存ファイル `app/core/config.py` への2フィールド追加だけが漏れ、`pytest tests/unit/test_structuring_service.py` 実行時に初めて `AttributeError` で発覚(pydantic-settings は未定義フィールド参照を型チェックでは検出できない)。対応: samples と同じ配置・値(`GEMINI_MODEL` の後)で `STRUCTURE_RATE_LIMIT_PER_HOUR: int = 20` / `_PER_DAY: int = 100` を追加。Phase 9-8 の `on_startup` への `pool_pre_ping` 追記漏れ(Q58)と同型 ──「新規ファイルが依存する既存ファイルへの副次的な追記」は写経で見落としやすい。
- **Phase 11-8 ── `.env` を編集しても稼働中コンテナに反映されず `GOOGLE_API_KEY` が空のまま → `/structure` が毎回 502** — `docker-compose.yml` の `env_file: .env` はコンテナ**作成時**の 1 回きりの注入で、`.env` を後から編集しても `uvicorn --reload`(ソース再読込のみ)はもちろん `docker compose restart` でも反映されない。原因: `_invoke_with_retry` が `ChatGoogleGenerativeAI` 構築時の `ValueError`(空キー)を一時的エラーとしてリトライし尽くし `GenerationFailedError`(`BadGatewayError`→502)に落ちる。対応: `docker compose up -d --force-recreate backend worker` でコンテナを作り直す。副次対応: `_invoke_with_retry` の `except Exception` 節と最終失敗時に `logger.warning`/`logger.error` を追加(以前は例外を握りつぶしログに何も残らず、`AppError` は登録済みハンドラで処理されるため uvicorn の「未処理例外」ログにも乗らなかった)。教訓 = `AppError` 集約ハンドラ(良い設計)は例外を握りつぶして見えなくする副作用も持つ ── 例外を投げる箇所には最低限のログを対で書く。
- **Phase 11-9 ── `knapsack_dp` の性能問題(budget が大きいとタイムアウト連鎖) → `select_strategy` に規模ガードを追加** — travel_planning で「10万円以内で北海道旅行」のように LLM が `budget=100000` を抽出すると、`KnapsackDpTravelStrategy` の DP グリッド(`O(places数×floor(budget)×floor(time_budget))`)が肥大化し(実測: `cap_a=15000,cap_b=16,n=5→0.74秒` / `cap_a=100000,cap_b=16,n=5→5.23秒`)、`/benchmark` の `runs=3` 逐次実行や `/solve` が `SOLVE_TIMEOUT_SECONDS`(10秒)を超える。タイムアウトしても裏スレッドは止まらない(既存の MVP 割り切り)ため、ゾンビ化した計算が GIL を専有し後続の無関係な `/solve` まで巻き込んで連鎖する。対応: `app/services/algorithm_selection.py::_preferred_name` の travel_planning 分岐に `_MAX_KNAPSACK_DP_CELLS = 2_000_000`(暫定閾値)のガードを追加し、超える場合は `greedy` にフォールバック(`?algorithm=knapsack_dp` で明示指定すれば従来通り選べる)。`logistics_planning`(同じく既定 `knapsack_dp`)は今回の実インシデントの対象外のため触っていない。閾値は Phase 15 の性能テストで見直す可能性がある。
- **Phase 11-10 ── travel_planning は「地域」を一切見ておらず、どんな行き先を要求しても固定カタログ(浅草エリア)が返る(ユーザー指摘で発覚)** — Phase 11 の設計原則(Q59: 「LLM はカタログを発明しない、常にベース問題から引き継ぐ」)通りの挙動であり実装バグではないが、「北海道旅行」と入力しても浅草の地点情報が返るのは体験として破綻している。ユーザーから「この原則のままなら LLM を通す意味がない」との指摘があり、Phase 11 全体の再設計(LLM に travel_planning のカタログを都度生成させる方向への転換、フロントエンドでの構造化入力(目的地必須化・候補地数指定で性能问题も緩和)、`generate_catalog` ノードの追加案)を検討した。README の Phase 12〜14(アルゴリズム推薦/結果説明/LLM対比較)は Phase 11 のカタログ設計にほぼ非依存、Phase 15(性能テスト)は上記 Phase 11-9 と一部重複することを確認した上で、**この再設計は Phase 15 完走後に要件を再検討する**こととし、今回はコード変更を行わない(詳細 `q_a.md` 該当エントリ)。
- **Phase 11-11 ── `StructuringResponse.notes` が計算されるが API レスポンスに配線されておらず常に `[]`(調査で発覚、未修正)** — `assemble_problem`(`app/ai/graph/nodes.py`)は目的抽出失敗時などに `notes: list[str]` を構築するが、`ProblemStructuringService.structure()` は `(conversation, problem)` のみを返し、`app/api/routes/structure.py` も `StructuringResponse` 構築時に `notes` を渡していない ── 実 API では常に `notes=[]`。一方 UI 側(`StructuredProblemCard.tsx`)は既に `notes` を表示する前提で実装済み(死んだ導線)。Phase 11-10 の再設計時(カタログが LLM 推定である旨の注記等)に合わせて配線する。
- **Phase 11-12 ── 開発用「LLM の自然言語での理解を確認する」機能は要検討課題として保留** — `with_structured_output()` は既定で `json_schema` モード(`langchain-google-genai==4.3.3`)であり、モデルの応答全体が構造化 JSON になるため `include_raw=True` にしても自由記述の reasoning は得られない。実現するには要約用の別枠 LLM 呼び出し(トークン増)か、Gemini の thinking モード(chain-of-thought 的な `thought` ブロックが出るが「curated な説明」ではない)を有効化する必要がある。`DEBUG`/`ENVIRONMENT` 設定は既に存在するが、レスポンスフィールドを dev 限定で出し分ける前例はプロジェクトに無い(今回が最初のユースケースになる)。Phase 11-10 の再検討と合わせて着手する。
- **Phase 11-2 ── `seeds.py`(本番の参照データ)が既存 `scripts/seed.py`(dev用DBシーディング)と命名衝突した** — 「seed」は本プロジェクトで既に「DB に投入する開発用データ」の意味で確立していたため、別概念(毎リクエスト参照するテンプレート問題)に同じ語を使うとユーザーが誤読した(実際に質問された)。`app/domain/problems/base_problems.py`(`BASE_PROBLEMS`/`get_base_problem`)にリネームして解消。教訓 = 新しいモジュール名を決める前に `grep -rn` で語の既存用途を確認する(詳細 `q_a.md` Q59 フォローアップ)。
- **Phase 14 ── `ComparisonService` の最適性集計が route/network/travel/project/logistics で常に `None` になった** — 原因: `RunOutcome.metrics` に `verified.metrics` をそのまま詰めていたが、目的の対象値(`total_weight` 等)はこの5ドメインでは `assignments` 側のフィールドにあり `metrics` には乗らない(shift だけ `assignment_metrics()` 経由で `metrics` にある)。対策: `_run_once` に `_objective_metrics()` を挟み、`metrics` に無ければ `getattr(solution.assignments, target)` で補う。
- **Phase 14 ── `constraint_compliance_rate_llm` の分母を全試行数(`attempts`)にしていた** — 2回中1回エラー・1回 valid のテストで期待値 `1.0`(成功した試行のうちの遵守率)に対し `0.5` が返り発覚。対策: 分母を「成功した試行数」(`ok`)に修正(エラーは遵守率でなく `error_rate_llm` 側に反映する設計に合わせた)。
- **Phase 14 ── 実際に Gemini で `POST /api/v1/compare` を叩くと6ドメイン全滅した(`FakeLLM` のユニットテストでは発見できない)** — `RouteSolution` 等の判別子 `problem_type: Literal[...] = "..."` をそのまま `with_structured_output()` に渡すと、Pydantic が単一値 Literal を JSON Schema の `"const"` で表現するが Gemini の構造化出力は `const` をサポートせず制約が無視され、LLM が無関係な値(実測 `"shortest_path"`)を生成して `literal_error`。対策: `app/algorithms/llm/common.py::strip_problem_type()` で判別子を除いたスキーマを動的生成して LLM に渡し、`solve()` 側で固定値を足し戻す(既存6スキーマの再利用という設計自体は不変)。
- **Phase 15-2 ── `measure_call`(`tracemalloc` 計装)は大量の小オブジェクト割当を行うアルゴリズムで10倍以上のオーバーヘッドを生む** — `knapsack_2d` の復元用3次元配列(`take[a][b] = list(...)`)で実測: 素の `time.perf_counter` 1.84秒 に対し `tracemalloc` 計装ありで20.6秒。`_MAX_KNAPSACK_DP_CELLS` は `POST /solve`(計装なし)しか保護しておらず `POST /benchmark`(`measure_call` 経由)は無関係と判明 ── 性能ガードを実測するときは「対象が実際にどの経路(計装あり/なし)を通るか」を先に確認する。
- **Phase 15-3 ── `topological_sort`(DFS)は線形依存チェーンで n≈999 から `RecursionError`** — Python既定の再帰上限(1000)に対し、枝分かれの無い直列依存では再帰の深さがそのまま n になる。`Phase-8-1.md` が理論として予告していたKahn法への置換を実測で正式にトリガー、反復版へ置換(`has_cycle`/`successors_from_edges` は無改造、下流60テストは出力順アサーション1件のみ更新)。
- **Phase 15-5 ── CPython(GIL あり)では `asyncio.to_thread` で CPU バウンドな solve() を並列化しても真の並列化はされない** — 同じ knapsack DP を `ThreadPoolExecutor` で同時実行数を振って実測すると、壁時計時間はほぼ同時実行数に比例して伸びる(10並列で単発の約10.4倍)。arq `WorkerSettings.max_jobs`(既定10)を4に下げ、Phase 11-9 のゾンビ化(タイムアウトしても裏スレッドが止まらない)による GIL 専有の連鎖リスクを抑えた。
- **Phase 15-11 ── 本番 `docker-compose.prod.yml` に `worker`(arq)サービスが無かった** — 開発用 `docker-compose.yml`(Phase 9-8)には worker があるが、本番用は同期 solve の `backend` サービスのみ。`POST /jobs`/`POST /simulate` は受理されるが処理されず `queued` のまま無応答になる実害のあるギャップ ── Phase 15 の Deployment 監査で発見し追加。テンプレート資産だけでなく DeciTima 固有の追加分(Phase 6〜14)が本番構成に反映されているかも監査対象にする。
- **Phase 15-1 追補(ユーザー写経で発覚) ── Docker コンテナに analysis 依存群(pandas/matplotlib)が無く `tests/analysis/test_plots.py` の collection が `ImportError` で中断** — Docker イメージは README §8 の境界どおり analysis 依存群を意図的に含めない設計だが、`tests/analysis/` 自体が `integration`/`performance` と同型のマーカー除外の対象外になっていた漏れ。`analysis` マーカー(`tests/analysis/conftest.py::pytest_collection_modifyitems` が自動付与)を新設し `addopts` に `not analysis` を追加して解消。**`pytest_collection_modifyitems` は「歴史的フック」で、conftest.py の置き場所に関わらず収集された items 全件を受け取る**(ディレクトリスコープの自動絞り込みは無い)── 最初の実装で `item.path` による絞り込みを忘れ、全テストに `analysis` マーカーが付いて既定実行が0件収集になる事故を起こした(overlay の `no tests collected` で発覚)。
