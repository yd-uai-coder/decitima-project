...
ライブラリ依存は必要になった Phase まで追加を遅延する: `networkx`(Phase 4、手実装グラフアルゴリズムの検証オラクル兼)、`ortools`(Phase 6、Phase 8/9 で再利用)、`numpy`(Phase 3 ベンチマーク集計)、`pandas` / `matplotlib`(Phase 3、分析トラック `analysis/` の dev 依存)、`pulp` / `scipy`(Phase 9 で MILP 化する場合)。
...
- (Claude 観察 / Q31 → Q32 で言語化)**共通化するのは「機構」であって「オーケストレーション」
  ...
  Q32 = 機構 vs オーケストレーション)── 教材が積み上がるほど新 Phase の判断コストが下がる。
