...
- **Phase 8 のトポロジカルソートは DFS ベース**（README §19「Phase 1 の DFS が土台」+ `search/dfs.py` の再利用 + 3 色による閉路検出の教材価値）。Kahn 法（入次数 BFS ── 反復で再帰上限に当たらない / CPM 前進パスと融合可能 / 辞書順出力）への置換は、依存 DAG が大規模化する工程管理では技術的には適するが、**プロジェクト完成後（Phase 15 以降）に検討する課題**。置換時の変更範囲は `topological.py` + `test_topological_sort.py` + `Phase-8-1.md` §1/§2（`has_cycle` / `successors_from_edges` の公開シグネチャは不変にできる）。経緯は `q_a.md` Q46。
