...
- MVP(Phase 0〜6)完了時点の課題と対応の一覧・継続課題の整理・提案の採否状況は
  `textbook/appendix/cl-development-retrospective.md`。
...
- 実装着手前に「実装前チェックリスト」(作るファイル / 各クラス・関数の責務 / テスト観点)で
  疑問を出し切る。
- 質問と回答は `textbook/q_a.md`(保存用)に、後の Phase に効く決定・知見は `CLAUDE.md`
  「### 設計判断・検証知見」に、進行方法の振り返りは
  `textbook/appendix/cl-development-retrospective.md` に記録する。
